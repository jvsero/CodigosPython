# 👋 Olá, eu sou Joel!

### 🚀 Nosso lema é crescer juntos.

> **Sua chegada só reforça isso.**
> Boas-vindas ao time! 🤝

<div align="center">
  <img height="300" src="https://media0.giphy.com/media/v1.Y2lkPTc5MGI3NjExenBpMmdxYXBkNjZ0bDUyZ2U3aWVjdWExcG14eGx0MTMwcGU5bTg4ayZlcD12MV9pbnRlcm5hbF9naWZfYnlfaWQmY3Q9Zw/3oEjHWpiVIOGXT5l9m/giphy.gif" />
</div>

---

## 👨‍💻 Sobre mim

Sou apaixonado por **tecnologia, programação e desenvolvimento de sistemas**.

Atualmente estou ampliando meus conhecimentos em programação, explorando diferentes tecnologias e colocando o aprendizado em prática através de projetos.

🎯 **Objetivo:** aprender continuamente, desenvolver projetos e evoluir junto com a comunidade.

---

## 🛠️ Tecnologias e linguagens

<div align="center">

### Linguagens

<img src="https://skillicons.dev/icons?i=python,php,javascript,html,css" />

### Banco de dados

<img src="https://skillicons.dev/icons?i=mysql,postgresql,mariadb" />

### Ferramentas e ambiente

<img src="https://skillicons.dev/icons?i=git,github,linux,vscode,docker" />

</div>

---

## 📊 Linguagens mais utilizadas

<div align="center">

<!-- Substitua SEU_USUARIO pelo seu nome de usuário do GitHub -->

<img
src="https://github-readme-stats.vercel.app/api/top-langs/?username=SEU_USUARIO&layout=donut&theme=tokyonight&hide_border=true&langs_count=8"
alt="Linguagens mais utilizadas"
/>

</div>

### 📈 Distribuição dos códigos

O gráfico acima apresenta a **porcentagem aproximada de código por linguagem** encontrada nos meus repositórios públicos do GitHub.

> 💡 A porcentagem é calculada automaticamente pelo GitHub Readme Stats e pode mudar conforme novos projetos e códigos são adicionados.

---

## 📌 Estatísticas do GitHub

<div align="center">

<!-- Substitua SEU_USUARIO pelo seu nome de usuário do GitHub -->

<img
height="180"
src="https://github-readme-stats.vercel.app/api?username=SEU_USUARIO&show_icons=true&theme=tokyonight&include_all_commits=true&count_private=true&hide_border=true"
alt="Estatísticas do GitHub"
/>

<img
height="180"
src="https://github-readme-stats.vercel.app/api/top-langs/?username=SEU_USUARIO&layout=compact&theme=tokyonight&hide_border=true"
alt="Principais linguagens"
/>

</div>

---

## 🔥 Contribuições

<div align="center">

<img
src="https://streak-stats.demolab.com?user=SEU_USUARIO&theme=tokyonight&hide_border=true"
alt="GitHub Streak"
/>

</div>

---

## 💡 Filosofia

<div align="center">

> **"O conhecimento cresce quando é compartilhado."** 🌱

**Aprender → Praticar → Compartilhar → Evoluir**

</div>

---

<div align="center">

### 🚀 Obrigado pela visita!

<img height="400" src="https://media0.giphy.com/media/v1.Y2lkPTc5MGI3NjExcDRuNTNtZDRnczQzYnMwYTdpdzlzZTRpb2IxeGh0cWU0OTRzeTVjcSZlcD12MV9pbnRlcm5hbF9naWZfYnlfaWQmY3Q9Zw/coxQHKASG60HrHtvkt/giphy.gif" />

</div>

---

<div align="center">

⭐ **Se algum projeto meu for útil para você, considere deixar uma estrela!**

</div>
